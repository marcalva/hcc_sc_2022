...
fn <- paste0("exp/sharma_aiz/markers/markers.res.1.txt")
mk <- read.table(fn, header=TRUE)

fn <- paste0("exp/sharma_aiz/markers/markers.res.1.log_fc.txt")
lfc <- read.table(fn, header=TRUE, check.names=FALSE, row.names=NULL)
k <- !is.na(lfc[,2])
lfc <- lfc[k,]
rownames(lfc) <- lfc[,"row.names"]

dir_exp <- "exp/sharma_aiz/path_enr/"
create_dir(dir_exp)

rc <- integrated@assays$RNA@counts
md <- integrated@meta.data
cts <- sort(as.character(unique(md[,"integrated_snn_res.1"])))
...
outfn <- paste0(dir_exp, "markers.res.1.path_enr.rds")
...
for (a in names(enr_all)){
    ...
    outfn <- paste0(dir_exp, "markers.res.1.", a, ".txt")
...
outfn <- paste0(dir_exp, "markers.res.1.path_gse.rds")
...
for (a in names(gse_all)){
    ...
    outfn <- paste0(dir_exp, "markers.res.1.gse.", a, ".txt")
