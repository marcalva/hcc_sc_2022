...
seur <- readRDS("data/processed/d_sct_cca_all/hep1/seur.cca.rds")
...
feat1 <- setdiff(all_feat, np_feat)

# Remove droplets from res1 clusters 10 and 12
k <- seur$integrated_snn_res.1 %in% c(10,12)
# k <- seur$pct.npc > midp[3]
cells1 <- rownames(seur@meta.data)[!k]
seur_s <- subset(seur, cells = cells1)
DefaultAssay(seur_s) <- "RNA"
...
seur_s.l <- lapply(seur_s.l, SCTransform, vars.to.regress = "pct.npc")
...
p <- plot_umap_c(seur_s,
                 ...
                 size = 0.3,
                 alpha = 0.5)
ggsave(paste0(dir_plot, "UMAP.pct_npc.jpeg"),
       width = 5,
       height = 5,
       units = "in")
...
mall$npc <- mall[,"gene"] %in% np_feat
tapply(mall$npc, mall$cluster, table)
