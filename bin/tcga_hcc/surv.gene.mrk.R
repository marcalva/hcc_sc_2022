...
dir_exp_main <- "exp/tcga_hcc/survival.gene/cell_type_main/"
dir.create(dir_exp_main, showWarnings = FALSE, recursive=TRUE)

dir_exp_fine <- "exp/tcga_hcc/survival.gene/cell_type_fine/"
...
# hazard ratios
fn <- paste0(dir_exp, "cox_ph.all_genes.txt")
all_gene_hr <- read.table(fn, header=TRUE, row.names=1)

fn <- paste0(dir_exp, "cox_ph.stage_all_genes.txt")
...
for (cl_id in cl_ids){
    ...
    kg <- ct_mrk[,"gene"] %in% rownames(all_gene_hr)
    ct_mrk <- ct_mrk[kg,]
...
for (cl_id in cl_ids){
    ...
    for (ct in cell_types){
        ...
        for (e in events){
            ...
            ct_g <- ct_mrk_s[,"gene"]
            hr_col <- paste0("cdr.", e, ".exp_coef")
            p_col <- paste0("cdr.", e, ".p_adj")
...
for (cl_id in cl_ids){
    ...
    for (ct in cell_types){
        ...
        ct_g <- ct_mrk_s[ct_mrk_s[,"cluster"] == ct,"gene"]
        ct_g_n <- length(ct_g)
        e_l <- list()
        for (e in events){
            pv_col <- paste0("cdr.", e, ".p_adj")
            cf_col <- paste0("cdr.", e, ".exp_coef")
