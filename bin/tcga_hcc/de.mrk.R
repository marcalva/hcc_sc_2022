...
dir_exp <- "exp/tcga_hcc/de/"
dir.create(dir_exp, showWarnings = FALSE, recursive=TRUE)

dir_exp_main <- "exp/tcga_hcc/de/cell_type_main/"
dir.create(dir_exp_main, showWarnings = FALSE, recursive=TRUE)

dir_exp_fine <- "exp/tcga_hcc/de/cell_type_fine/"
...
g_exprd <- gsub("\\..*", "", g_exprd)
gk <- g_exprd %in% rownames(de)
g_exprd <- g_exprd[gk]
...
#================================================
# add DE stats to single cell marker data
#================================================
...
for (cl_id in cl_ids){
    ...
    for (ct in cell_types){
        ...
        ct_mrk_s[,"de_p_adj"] <- de[ct_g, "p_adj"]
        d1_l[[ct]] <- ct_mrk_s
    }
    mrk_de_l[[cl_id]] <- d1_l
}
...
# genome-wide significant colors
gmap <- c(" " = "black", "adj p < 0.05" = "red")

# subset
pdatf <- pdatf[pdatf[,"avg_log2FC"] > mrk_logfc_thr,]

p <- boxj_plot(pdatf, title = paste0("TCGA"), gmap = gmap)
