...
# Set directories
dir_plt <- "exp/aizarani2019/sct_clust/plots/";
...
meds <- get_med_points(udf, c("UMAP1", "UMAP2"), "CellType")

p <- ggplot(udf, aes(x=UMAP1,y=UMAP2,color=CellType)) +
geom_point(shape=16, size=.01) +
scale_color_manual(values=pal30, name="Fine") +
ggtitle("Aizarani") + 
theme_s + 
...
outfn <- file.path(dir_plt, "UMAP.CellType.pdf")
...
meds <- get_med_points(udf, c("UMAP1", "UMAP2"), "Major")

p <- ggplot(udf, aes(x=UMAP1,y=UMAP2,color=Major)) +
geom_point(shape=16, size=.01) +
scale_color_manual(values=pal30, name="Main") +
ggtitle("Aizarani") + 
theme_s + 
guides(color = guide_legend(override.aes = list(size=1))) + 
geom_text_repel(data=meds, aes(x=UMAP1, y=UMAP2), 
...
outfn <- file.path(dir_plt, "UMAP.Major.pdf")
ggsave(outfn, width = 3.5, height = 3, dpi=300)
pname <- gsub("pdf$", "png", outfn)
ggsave(pname, width = 3.5, height = 3, dpi=300)
